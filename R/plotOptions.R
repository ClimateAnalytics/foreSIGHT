...
plotOptions <- function(performanceOpt1, # system model performance for option 1, matrix of size targets x replicates
  ...
  if (is.list(performanceOpt1) & is.list(performanceOpt2)) {
    ...
    performanceOpt1[[1]] <- perfMat
    rm(perfMat)
    perfMat <- performanceOpt2
    performanceOpt2 <- list()
    performanceOpt2[[1]] <- perfMat
    rm(perfMat)
  } else {
  ...
  if (!is.null(perfThresh)) {
    ...
    p1 <- addThresholdLines(p1, opt1Mean, perfThresh,
      perfThreshLabel = paste0(opt1Label, " ", perfThreshLabel), names(opt1Mean)[1:2], names(opt1Mean)[3],
    ...
    p1 <- addThresholdLines(p1, opt2Mean, perfThresh,
      perfThreshLabel = paste0(opt2Label, " ", perfThreshLabel), names(opt2Mean)[1:2], names(opt2Mean)[3],
