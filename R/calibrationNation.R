...
modCalibrator <- function(obs = NULL,
  ...
  argCheck <- argument_check_calibrator(
    names = names(obs), # Need an invalid modelTage Choice flag if FS type
    obs = obs,
    modelTag = modelTag
  )
  ...
  for (i in 1:length(modelTag)) {
    if (is.null(obs$P)) {
      ...
      rain <- obs$P
    } # little temp switch here

    tempVar <- modelInfo[[modelTag[i]]]$simVar

    parOut <- init.calib.master(
      modelTag = modelTag[i],
...
init.calib.Pwgen <- function(modelTag = NULL, # model identifier
  ...
  pdd <- rep(0, modelInfo$nperiod)
  alpha <- beta <- pwd <- pdd # make space to store fitted pars
  ...
  for (p in 1:modelInfo$nperiod) {
    ...
    if (modelTag == "P-har-wgen") { # This will still fit harmonics daily, (i.e. modelInfor$nperiod=365), but the indexing needs to include a window of data either side of the single day
      ...
      for (j in 1:length(datInd$i.pp[[p]])) { # extend each index (one point each year) to include window days either side
        ...
        if (j == 1) {
          datarun <- datarun[datarun > 0]
        } # in first and final years, trim data, as sometimes cannot grab from left or right, respectively
        if (j == length(datInd$i.pp[[p]])) {
          datarun <- datarun[datarun <= utils::tail(datInd$i.pp[[modelInfo$nperiod]], n = 1)]
        }
  ...
  if (!is.na(modelInfo$ncycle)) {
    ...
    initCalibPars <- c(
      unlist(pdd.fit, use.names = FALSE),
...
pdd.pwd.estimator <- function(dat = NULL, # vector of rainfall values
  ...
  ind.prior <- ind[-n]
  ind.now <- ind[-n] + 1 # for clarity spell out which is the prior day series and current day series (DROP LAST VALUE TO AVOID ARRAY OVERFLOW)
  ...
  if (modelInfo$WDcondition == FALSE) { # IF NO WET-DRY SEPARATION
    ...
    switch(modelInfo$wdCycle,
      "All" = {
        ...
        genRes <- rep(NA, datInd$nTimes) # MAKE BLANK VECTOR FOR RESIDUAL TS
        for (p in 1:modelInfo$nperiod) {
          genRes[datInd$i.pp[[p]]] <- calcDayResidFunc_wdSep(
            data = data[datInd$i.pp[[p]]],
...
calcDayResidFunc_wdSep <- function(data = NULL,
                                   ...
                                   threshold = NULL) {
  nLen <- length(rain)
  err <- rep(NA, nLen) # make err vector
