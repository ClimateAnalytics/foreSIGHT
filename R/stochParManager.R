...
#       #stop("Error: nperiod is not equal to 1, rep to create parameters of length `ndays` in the par manager will not work")
#       #check length(pars == modelInfo$npars)   # if it fails put in a warning
#       alpha <- rep_len(parS[1:modelInfo$nperiod], datInd$nTimes)                                 # extract first set of pars (pars evenly split across vector)
#       sigma <- rep_len(parS[(modelInfo$nperiod+1):(2*modelInfo$nperiod)], datInd$nTimes)
#       mu <- rep_len(parS[(2*modelInfo$nperiod+1):(3*modelInfo$nperiod)], datInd$nTimes)
#       lambda <- rep_len(parS[(3*modelInfo$nperiod+1):(4*modelInfo$nperiod)], datInd$nTimes)
#     } else if(modelInfo$nperiod == 4) {
#       if(datInd$ndays != (length(unlist(datInd[["i.ss"]])))) stop("seasonal parameter assignment doesn't work properly.")
