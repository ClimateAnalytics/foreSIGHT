...
attHold <- c("P_day_Feb_tot_m", "P_day_all_nWet_m", "P_day_all_R10_m", "P_day_SON_dyWet_m",
             ...
             "P_day_DJF_avgDSD_m", "Temp_day_all_rng_m")

# attHold <- c("P_day_Feb_tot_m",
#              "P_day_JJA_avgWSD_m","Temp_day_all_rng_m")
...
control = list()
control[["optimisationArguments"]] <- list()
control[["optimisationArguments"]][["nMultiStart"]] <- 1
# control[["optimisationArguments"]][["optimizer"]] <- 'NM'
#control[["optimisationArguments"]][["optimizer"]] <- 'optim.LBFGSB'
control[["optimisationArguments"]][["optimizer"]] <- 'RGN'
controlJSON = jsonlite::toJSON(control, pretty = TRUE, auto_unbox = TRUE)
controlFile = paste0(tempdir(), "\\eg_controlFile.json")
write(controlJSON, file = controlFile)


# perform stochastic simulation
simStochastic <- generateScenarios(reference = tank_obs,
                                   expSpace = expSpace,
                                   simLengthNyrs = 30,seedID = 1,numReplicates = 1,controlFile = controlFile)
...
o = evaluate_system_metrics(sim=simStochastic,
                        clim=clim,
                        systemModel=systemModel,
                        systemArgs=systemArgs,
                        metrics=metrics)
...
for (metric in metrics){
  boxplot(as.vector(o$systemPerf_base[[metric]]))
  points(x=1,y=o$systemPerf_obsClim[[metric]])
  title(metric)  
}
