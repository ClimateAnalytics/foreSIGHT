...
It is assumed that the reader has already read the [***Quick Start Guide*****vignette**]{.underline}, and is familiar with the basic *fore*SIGHT work flow demonstrated below:

```{r CRAFTWorkflow, echo=FALSE, fig.cap="Workflow of climate 'stress-testing' using *fore*SIGHT", out.width = '30%'}
...
: *Table 1: List of attribute functions available in* fore*SIGHT.*
...
## 4.3. S**tochastic simulation options (the control file)**
...
: *Table 3: Description of models available in* fore*SIGHT.*
...
```{r modOptimArgs, R.options = list(width = 100), eval = TRUE}
...
```{r modOptimArgs1, R.options = list(width = 100), eval = TRUE}
# add user-specified values for optimisation arguments
controlFileList[["optimisationArguments"]] <- list()
...
For computational simplicity, the example below targets a single point from the exposure space and generates a single stochastic climate replicate. The code is expected to run in approximately one minute.

```{r sim.stoch.single, eval=TRUE}
...
modelSelection[["penaltyWeights"]] = c(4,4,4,2,2)
...
# consider only a single target in exposure space
expSpaceTmp = expSpace
expSpaceTmp$targetMat = expSpace$targetMat[1,]

###############
# generate scenarios

time.1 = Sys.time()
sim.stoch.single = generateScenarios(reference = clim_ref,
                       expSpace = expSpaceTmp,
...
Simulated climates for the full exposure space, and with 50 replicates, have been pre-computed and can be loaded as follows:

```{r}
data("egScottCreekSimStoch") # this loads object sim.stoch
```
...
[**XXX We have pre-computed simulated climates for larger 5x5 exposure space, with 10 replicates:**]{.underline}
...
[**XXX Evaluate how well the target attributes are captured by the simulated climates:**]{.underline}
...
# load dates, precip, PET and streamflow data for Scott Creek
data('data_A5030502_1976_1985')
clim_ref = list(times = data$times,P = data$P)  
...
-   The black line shows the median of the simulated attribute values, while the grey band indicates the 90% range (5th to 95th percentiles).
-   Red lines highlight attributes that change significantly more than the intended perturbation (i.e. the slope of attribute vs. perturbed attribute \> 1.5).
-   Red crosses mark attributes with large biases—greater than 20%—relative to their observed (unperturbed) historical values.

The results show that [**XXXXX**]{.underline}
...
## 5.3. Evaluating ability of SWG to capture climate features relevant to system model XXX

Crosses over between perturbed stochastic climates and system response.

Want stochastic model
...
eval = evaluate_system_metrics(sim = sim.multiRepsLarge,
                        clim = clim_ref,
                        systemModel = GR4J_wrapper,
                        systemArgs = systemArgs,metrics = metrics)
...
# [6. Evaluate performance XXX]{.underline}

[**Step D**]{.underline}

[Plot OAT performance:]{.underline}

```{r, GR4J_OATplotsSeasRatio, fig.align = 'center', fig.width = 10, fig.height = 10, eval=TRUE}
# performance for changes in P_day_all_seasRatioMarMay (1st attribute)
plotPerformanceOAT(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'meanQ',attSel=attPerturb[1],topReps=40)
plotPerformanceOAT(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'P99',attSel=attPerturb[1],topReps=40)
plotPerformanceOAT(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'P25',attSel=attPerturb[1],topReps=40)
```

```{r, GR4J_OATplotsP99, fig.align = 'center', fig.width = 10, fig.height = 10, eval=TRUE}
# performance for changes in P_day_all_P99 (2nd attribute)
plotPerformanceOAT(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'meanQ',attSel=attPerturb[2],topReps=40)
plotPerformanceOAT(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'P99',attSel=attPerturb[2],topReps=40)
plotPerformanceOAT(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'P25',attSel=attPerturb[2],topReps=40)
```
...
plotPerformanceSpace(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'meanQ',type='filled.contour',nContour=5,topReps = 40)
plotPerformanceSpace(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'P99',type='filled.contour',nContour=5,topReps = 40)
plotPerformanceSpace(performance = sysOutSim, sim=sim.multiRepsLarge, metric = 'P25',type='filled.contour',nContour=5,topReps = 40)
```
...
This vignette has demonstrated how stochastic weather generators can be used within *fore*SIGHT to generate perturbed climates for stress testing water systems. Unlike simple scaling approaches introduced in the *Quick Start* vignette, stochastic generation enables exploration of a much broader range of climate attributes, offering a more comprehensive assessment of system vulnerabilities.
...
# **Appendix: Case Study Methods – Rainfall-Runoff Response in Scott Creek, South Australia**
